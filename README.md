...
Наконец то додумался синкретические данные сделать по подобию самого датасета, если бы сделал это раньше сейчас бы не пришлось адаптировать.
...
| `node_id_check`      |  Работает(теперь точно)| Адаптирован: теперь ищет отсутствие `node_id` при наличии других признаков контента. Находит. Проблема была в самом детекторе. |
...
| `page_view`          | Работает | Адаптирован, но в этих данных находит пока только skip |
| `isolation_forest`   | Работает | Невозможно обучить модель на данных с `NaN`, `inf` и отсутствием нужных колонок (например, `requests`). |
| `users_devices`      |  Работает | Метод протестирован, немного адаптирован и работает гладко |

**Проблемы, тормозящие оставшиеся методы:**
 # NONE
| Детектор           | Проблемы |
|--------------------|----------|
| `page_view`        | `page_view_order_number` — содержит `NaN` или `inf`, `randPAS_user_agent_id` — иногда `list` (не hashable) ---DONE| 
...
| `node_id_check`    | Адаптирован, не находит конфликты с отсутствием node_id и присутствием описания контента.---DONE| 
